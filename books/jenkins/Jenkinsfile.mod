pipeline {
    ...
    environment {
        ...
        DEV_PROJECT = "aedward-books-dev"
        STAGE_PROJECT = "aedward-books-stage"
        APP_GIT_URL = "https://github.com/aedward/DO288-apps"
        NEXUS_SERVER = "http://nexus-common.apps.ocp-ap3.prod.nextcle.com/repository/nodejs"
    ...
    stages {
        ...
        stage('Launch new app in DEV env') {
            steps {
                ...
                sh'''
			oc project ${DEV_PROJECT}
			oc new-app --name books nodjs:8~${APP_GIT_URL} \
			--build_env npm_config_registry=${NEXUS_SERVER} \
